...
export const jobs = pgTable('jobs', {
  ...
  include_patterns: text('include_patterns')
    .array()
    .notNull()
    .default(sql`'{}'`),
  exclude_patterns: text('exclude_patterns')
    .array()
    .notNull()
    .default(sql`'{}'`),
...
export const job_pages = pgTable(
  'job_pages',
  {
  ...
  (t) => [index('job_pages_job_id_idx').on(t.job_id)],
);
...
export const monitor_pages = pgTable(
  'monitor_pages',
  {
    id: serial('id').primaryKey(),
...
export const monitor_changes = pgTable('monitor_changes', {
  ...
  check_id: integer('check_id')
    .notNull()
    .references(() => monitor_checks.id, { onDelete: 'cascade' }),
...
export const scrape_requests = pgTable(
  'scrape_requests',
  {
    id: serial('id').primaryKey(),
    url: text('url').notNull(),
  ...
  (t) => [index('scrape_requests_created_at_idx').on(t.created_at)],
);
